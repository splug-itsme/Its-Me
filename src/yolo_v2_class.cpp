...
YOLODLL_API std::vector<bbox_t> Detector::detect(std::string image_filename, float thresh) // detect1번 이미지파일이름을 받아와 읽고 detect2번 실행
{
...
YOLODLL_API std::vector<bbox_t> Detector::detect(image_t img, float thresh) // detect2번
{
	...
	//draw_detections2(im, l.w*l.h*l.n, thresh, detector_gpu.boxes, detector_gpu.probs, l.classes);
	save_image(im, "predictions");
	std::vector<bbox_t> bbox_vec;

	for (size_t i = 0; i < (l.w*l.h*l.n); ++i) {
	/*	if (detector_gpu.probs[i] != 0)
			continue;*/
